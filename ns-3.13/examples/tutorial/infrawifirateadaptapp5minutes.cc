...
*/
	...
	std::cout << "Avergae Now NOW NOW	" << avgReTxCounter << std::endl;
...
//  ApplicationContainer apps = server.Install (wifiStaNodes.Get (nWifi - 1));
  ...
  appsClient.Stop (Seconds (simulationEndTime + 0.001));
  ...
  Simulator::Stop (Seconds (simulationEndTime + 0.001));
