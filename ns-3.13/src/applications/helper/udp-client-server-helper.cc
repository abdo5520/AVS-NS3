...
#include "ns3/rate-adaptive-sender-app.h"
...
// Added by Abdallah Abdallah to support RateAdaptiveSenderHelper
RateAdaptiveSenderHelper::RateAdaptiveSenderHelper ()
{
}
...
void
RateAdaptiveSenderHelper::SetAttribute (std::string name, const AttributeValue &value)
{
  m_factory.Set (name, value);
}

ApplicationContainer
RateAdaptiveSenderHelper::Install (NodeContainer c)
{
  ApplicationContainer apps;
  for (NodeContainer::Iterator i = c.Begin (); i != c.End (); ++i)
    {
      Ptr<Node> node = *i;
      Ptr<RateAdaptiveSender> client = m_factory.Create<RateAdaptiveSender> ();
      node->AddApplication (client);
      apps.Add (client);
    }
  return apps;
