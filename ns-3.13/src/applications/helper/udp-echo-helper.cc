...
#include "ns3/rate-adaptive-receiver.h"
...
/**
 * Added by Abdallah Abdallah on June 1, 2012
 * to support RateAdaptiveReceiverHelper
 */
RateAdaptiveReceiverHelper::RateAdaptiveReceiverHelper (uint16_t port)
{
  m_factory.SetTypeId (RateAdaptiveReceiver::GetTypeId ());
  SetAttribute ("Port", UintegerValue (port));
}

void
RateAdaptiveReceiverHelper::SetAttribute (
  std::string name,
  const AttributeValue &value)
{
  m_factory.Set (name, value);
...
ApplicationContainer
RateAdaptiveReceiverHelper::Install (Ptr<Node> node) const
{
  return ApplicationContainer (InstallPriv (node));
}

ApplicationContainer
RateAdaptiveReceiverHelper::Install (std::string nodeName) const
{
  Ptr<Node> node = Names::Find<Node> (nodeName);
  return ApplicationContainer (InstallPriv (node));
}
...
ApplicationContainer
RateAdaptiveReceiverHelper::Install (NodeContainer c) const
{
  ApplicationContainer apps;
  for (NodeContainer::Iterator i = c.Begin (); i != c.End (); ++i)
    {
...
Ptr<Application>
RateAdaptiveReceiverHelper::InstallPriv (Ptr<Node> node) const
{
  Ptr<Application> app = m_factory.Create<RateAdaptiveReceiver> ();
  node->AddApplication (app);
